...
$(function() {
  ...
  function render() {
    ...
    var htmlStr = template( "historyTpl", { arr: history } );
  ...
  $('.lt_history').on("click", ".btn_del", function() {
    ...
    mui.confirm("你确定要删除该条记录嘛?", "温馨提示", ["取消", "确认"], function( e ) {
      if ( e.index === 1 ) {// 用户点击了索引为 1 的确认按钮
        ...
        // splice(1, 0, "haha")  在1的位置添加一条数据
        // splice(1, 1, "hehe")  在1的位置替换一条数据
        arr.splice( index, 1 );
        // 转成 jsonStr, 存到本地存储中
