        ...
        $.ajax({
            ...
            success: function( info ) {
                ...
                $('#paginator').bootstrapPaginator({
                    ...
                    // 配置分页按钮的大小，mini,small,normal,large
                    size: "normal",

                    // 配置每个分页按钮的显示文本
                    // 每个按钮在初始化的时候, 都会调用一次这个函数, 通过返回值设置显示文本
                    ...
                    // 配置每个分页按钮的提示信息(鼠标放在按钮上的提示信息)
                    // 每个按钮在初始化的时候, 都会调用一次这个函数, 通过返回值设置提示信息
                    ...
                    // 配置是否使用 bootstrap 内置的的tooltip(提示框组件)，默认false不使用
                    // 如果单独使用，需要引入bootstrap-tooltip.js插件，在bootstrap - JavaScript插件 - 工具提示
                    // 但是此处不需要引入，因为引入Bootstrap包就包括了此插件
